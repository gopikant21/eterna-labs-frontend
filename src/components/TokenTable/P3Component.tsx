...
export function P3Component() {
  const [slippage, setSlippage] = useState("10");
  const [priority, setPriority] = useState("0.005");
  const [bribe, setBribe] = useState("0.05");
  const [autoFee, setAutoFee] = useState(false);
  const [maxFee, setMaxFee] = useState("0.2");
  const [mevMode, setMevMode] = useState<"off" | "reduced" | "secure">(
    "secure"
  );
  const [rpc, setRpc] = useState("https://c..e.com");
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <DialogTrigger asChild>
            <div className="text-sm text-blue-400 cursor-pointer hover:text-blue-300 transition-colors">
              P3
            </div>
          </DialogTrigger>
        </TooltipTrigger>
        ...
        >
          <div className="">
            ...
            <div className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-800/70 transition-colors cursor-pointer">
              ...
              <span className="text-xs text-yellow-400">0.005</span>
            </div>
            <div className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-800/70 transition-colors cursor-pointer">
              <Users className="w-4 h-4 text-blue-400" />
              <span className="text-xs text-blue-400">0.05</span>
            </div>
            <div className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-800/70 transition-colors cursor-pointer">
